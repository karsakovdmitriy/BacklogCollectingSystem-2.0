...
test('Verify GitLab Integration Settings, simplified Project Group configuration, and entity imports', async ({ page }) => {
  ...
  // 5. Test importing Clients from GitLab
  await page.click('button:has-text("1. Клиенты")');
  await page.waitForTimeout(500);
  ...
  // Take screenshot of imported clients
  await page.screenshot({ path: '/home/jules/verification/screenshots/settings_gitlab_clients_imported.png', fullPage: true });
